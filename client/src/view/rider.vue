                            ...
                            <div class="div_filter">
                                ...
                                <div style="padding-left:30px;display:inline-block"><img src="../Images/Vector(2).png" >&nbsp;<span class="sort">Filter by:</span>&nbsp;&nbsp;
                                    <select class="filter" style="padding: 10px 80px 10px 10px ;" id="selOrder2" @change="showOrderTable('#selOrder2')">
                                        ...
                                        <option value="picked">Picked Orders</option>
                            ...
                            <div id="orderDiv">
                                <div>
                                    <table class="orderTab">
                                        <tr><th>Order Id</th><th>Customer</th><th>Phone Number</th><th>Order Date</th><th>Address</th><th>Location</th><th>Status</th><th>Order Details</th><th>Pick</th></tr>
                                        <tr v-for="order in prepared" :key="order.id"><td>{{order.id}}</td><td>{{order.username}}</td><td>{{order.number}}</td><td>{{order.time}}</td><td>{{order.address}}</td><td>{{order.userLocation}}</td><td>{{order.status}}</td><td><button class="pending" @click="viewOrderDetails(order.id)">Details</button></td><td><button @click="claimOrder(order.id)" class="delivering">Pick</button></td></tr>  
                                    </table>
                                </div>
                                <div>
                                    <table class="orderTab">
                                        <tr><th>Order Id</th><th>Customer</th><th>Phone Number</th><th>Order Date</th><th>Address</th><th>Location</th><th>Status</th><th>Order Details</th><th>Deliver</th></tr>
                                        <tr v-for="order in picked" :key="order.id"><td>{{order.id}}</td><td>{{order.username}}</td><td>{{order.number}}</td><td>{{order.time}}</td><td>{{order.address}}</td><td>{{order.userLocation}}</td><td>{{order.status}}</td><td><button class="pending" @click="viewOrderDetails(order.id)">Details</button></td><td><button @click="readyOrder(order.id)" class="claim">Deliver</button></td></tr>  
                                    </table>
                                </div> 
                                <div>
                                    <table class="orderTab">
                                        <tr><th>Order Id</th><th>Customer</th><th>Phone Number</th><th>Order Date</th><th>Address</th><th>Location</th><th>Status</th><th>Order Details</th></tr>
                                        <tr v-for="order in delivered" :key="order.id"><td>{{order.id}}</td><td>{{order.username}}</td><td>{{order.number}}</td><td>{{order.time}}</td><td>{{order.address}}</td><td>{{order.userLocation}}</td><td>{{order.status}}</td><td><button class="pending" @click="viewOrderDetails(order.id)">Details</button></td></tr>  
                                    </table>
                                </div> 
                                <div>
                                    <table class="orderTab">
                                        <tr><th>Order Id</th><th>Customer</th><th>Phone Number</th><th>Order Date</th><th>Address</th><th>Location</th><th>Status</th><th>Order Details</th></tr>
                                        <tr v-for="order in orders" :key="order.id"><td>{{order.id}}</td><td>{{order.username}}</td><td>{{order.number}}</td><td>{{order.time}}</td><td>{{order.address}}</td><td>{{order.userLocation}}</td><td>{{order.status}}</td><td><button class="pending" @click="viewOrderDetails(order.id)">Details</button></td></tr>  
                                    </table>
                                </div>                  
                            </div>
                        </div>
    ...
    export default{
        ...
        data(){
            return{
                ...
                users:[],orders:[], userId: 0,picked:[],delivered:[],prepared:[]
            }
        },
        mounted(){
            ...
            if(localStorage.getItem("user")== null){
                ...
                if(localStorage.getItem("level")==null || localStorage.getItem("level").localeCompare("rider") != 0){
                    location.href="/login";
                }
                document.getElementById("uName").innerHTML = localStorage.getItem("userN") ;
            } 
            this.showSelectedMenu('a_overview','#div_overview');
            this.getChefData(this.items,this.drinks,this.users,this.orders,this.prepared, this.picked, this.delivered)
        },
        methods:{
            ...
            getOrders(orders,prepared, picked, delivered,co){
                let r = [0,0,0,0]
                for(let i = co.orders.length-1; i>=0; i--){
                    if(co.orders[i].isChefOrder==1){
                        orders[r[0]++] = co.orders[i];
                        if(co.orders[i].status.localeCompare("picked")==0){
                            picked[r[1]++] = co.orders[i];
                        }
                        else if(co.orders[i].status.localeCompare("delivered")==0){
                            delivered[r[3]++] = co.orders[i];
                        }
                    }
                    else{
                        if(co.orders[i].status.localeCompare("prepared")==0){
                            prepared[r[2]++] = co.orders[i];
                            orders[r[0]++] = co.orders[i];
                        }
                    }
                }
            },
            getChefData(items,drinks,users,orders,prepared, picked, delivered){
                let dis = this;
                ...
                xhhtp.onreadystatechange = function(){
                    if (this.readyState == 4 && this.status == 200){
                        ...
                        dis.getOrders(orders,prepared, picked, delivered,co)
                        dis.showOrderTable("#selOrder");
                    }
                }
                xhhtp.open("GET", this.url + '/api/getriderdata/'+usId, true); 
                xhhtp.send();
            },
            changePwd(n){
                this.userId = n;
            ...
            claimOrder(n){
                ...
                xhhtp.open("POST", this.url + '/api/pickorder', true); 
            ...
            readyOrder(n){
                ...
                xhhtp.onreadystatechange = function(){
                    if (this.readyState == 4 && this.status == 200){
                        ...
                        if(co.status==1){
                            alert(co.message);
                            this.orders = []; this.prepared = []; this.picked = [], this.delivered = [];
                            this.getOrders(this.orders,this.prepared, this.picked, this.delivered,co)
                        }
                    }
                }
                xhhtp.open("POST", this.url + '/api/deliveredorder', true); 
                xhhtp.setRequestHeader("Content-type", "application/json"); 
                xhhtp.send(JSON.stringify(obj));
            },
            showOrderDetails(n){
